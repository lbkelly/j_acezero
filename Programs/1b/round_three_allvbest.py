...
def main(seed, exp_no, run_specs, test_set):
    ...
    if SEEDED == 1:
        pop_string = "r3_blue_biased_" + str(POP) + "_pop.pkl"
        with open(pop_string, "r") as cp_file:
            cp = cPickle.load(cp_file)
        blue_pop = cp["population"]

        pop_string = "r3_red_biased_" + str(POP) + "_pop.pkl"
        with open(pop_string, "r") as cp_file:
            cp = cPickle.load(cp_file)
    ...
    while i <= GENS:
        ...
        # if both HOFs are at full size
        if len(blue_hof) == HOF_SIZE and len(red_hof) == HOF_SIZE:
            # get best performing blue and red against respective HOFS
            blue_pop = helpers.sum_pop_v_hof_evals(blue_pop, red_hof)
            red_pop = helpers.sum_pop_v_hof_evals(red_pop, blue_hof)
            # use this fitness to select the 5 best of each
            blue_best_agents = tools.selBest(blue_pop, 5)
            red_best_agents = tools.selBest(red_pop, 5)
            # reset the fitnesses
            for index, x in enumerate(blue_pop):
                x.fitness.values = (0.0,)
            ...
            # eval whole pops against 5 best from the other
            blue_pop = helpers.sum_pop_v_hof_evals(blue_pop, red_best_agents)
            red_pop = helpers.sum_pop_v_hof_evals(red_pop, blue_best_agents)
        else:
        ...
        for index, x in enumerate(blue_pop):
            if len(blue_hof) == HOF_SIZE and len(red_hof) == HOF_SIZE:
                if HOF == 1:
                    x.fitness.values = (x.fitness.values[0] / (5 + len(red_hof)),)
                else:
                    x.fitness.values = (x.fitness.values[0] / 5,)
            else:
                if HOF == 1:
        ...
        for index, y in enumerate(red_pop):
            if len(blue_hof) == HOF_SIZE and len(red_hof) == HOF_SIZE:
                if HOF == 1:
                    y.fitness.values = (y.fitness.values[0] / (5 + len(blue_hof)),)
                else:
                    y.fitness.values = (y.fitness.values[0] / 5,)
            else:
                if HOF == 1:
        ...
        if i % 5 == 0:
            ...
            cp_blue = dict(population=blue_pop, generation=i, rndstate=random.getstate())

            with open(sub_folder_string + "zGen " + str(i) + "blue.pkl", "wb") as cp_file:
                cPickle.dump(cp_blue, cp_file)

            cp_red = dict(population=red_pop, generation=i, rndstate=random.getstate())

            with open(sub_folder_string + "zGen " + str(i) + "red.pkl", "wb") as cp_file:
                cPickle.dump(cp_red, cp_file)
        ...
        for index, b in enumerate(best_blue):
            blue_best_file.write("Chromosome [" + str(index) + "]: " + "(" + str(b.fitness.values[0]) + ")" + helpers.list_to_string(b) + "\n")

        for index, r in enumerate(best_red):
            red_best_file.write("Chromosome [" + str(index) + "]: " + "(" + str(r.fitness.values[0]) + ")" + helpers.list_to_string(r) + "\n")
    ...
    # stats = [seed, sum(stats_rel_b) / 10, sum(stats_bgp_b) / 10, sum(stats_agp_b) / 10, sum(stats_rel_r) / 10,
    #          sum(stats_bgp_r) / 10, sum(stats_agp_r) / 10]
